...
DEFAULT_FIELDS = {
    ...
    'password':None,'login_method':'cookie','profile_link':None,
        ...
        try:
            with conn.cursor() as cur:
                cur.execute('''CREATE TABLE IF NOT EXISTS accounts (
                  ...
                  profile_link TEXT, created_at TEXT)''')
...
def add_account(username, category='dance', platform='TikTok'):
    if not _pg_enabled():
        with _lock:
            ...
            _accounts[username]=_new_account(username,category,platform); return True
    conn=_pool.getconn()
    try:
        with conn.cursor() as cur:
            cur.execute('''INSERT INTO accounts(username,category,platform,status,current_task,logs,verify_code,login_method,created_at)
                           VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT DO NOTHING''',
                        (username,category,platform,'Disconnected','Idle','','','cookie',datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
